...
export const MainInput = ({disabled, onChange, value}) => {
    return (
        <div className="relative flex flex-col w-9/12 mx-auto h-12 border px-4 rounded-md font-bold text-gray-200 mb-10">
            ...
            <input value={value} id='amount'  onChange={(e)=>onChange(e.target.value)} type="text" disabled={disabled} className="h-full bg-transparent text-lg text-right outline-none"/>
        </div>
    )
}
